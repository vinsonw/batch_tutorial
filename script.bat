:: batch tutorial
:: source: https://www.youtube.com/watch?v=JbAyxKjUPs0&list=PL69BE3BF7D0BB69C4
...
:: in cmd shell: 
:: for %i in (banks.csv file.bat script.bat) do copy /y %i .\new_folder

rem E50//Delting files

:: del /? for help info
:: del /S *.txt -> delete txt file in all subdirectories
:: del /Q -> delete quietly

rem E51 //Color of the console
:: skip

rem E52 //Command Prompt Property
:: skip

rem E53 //Changing the Prompt
:: skip

rem E54 //Colormsg Script
:: script doesn't work properly, skipped
...
rem     :: start index: 8
rem     :: length: 100 (by default it's all chars after)
rem     echo !var:~8,45!
rem     :: get the last 6 characters
rem     echo !var:~-6,6!

rem goto :eof

rem E56 String Substitution
...
rem     set var=The cat in the hat ate the mat cat cat cat

rem     :: substitution
rem     set var=!var:cat=dog!
rem     set var=!var:hat=raincoat!
rem     echo !var!

rem     :: delete "raincoat"
rem     set var=!var:raincoat=!
rem     echo !var!
...
rem     :: about %%~g: expands %%g removing any surrounding quotes (")
rem     :: tokens=* means "grab everthing"
rem     :: usebackq        - specifies that the new semantics are in force,
rem     ::                       where a back quoted string is executed as a
rem     ::                       command and a single quoted string is a
rem     ::                       literal string command and allows the use of
rem     ::                       double quotes to quote file names in
rem     ::                       file-set.
rem     for /f "usebackq tokens=*" %%g in ('!string!') do (
rem        set string=%%~g
rem     )
rem     echo !string!
...
rem E58//Buidling create_string function
:: "Modules" in batch scripting
...
rem :main
rem setlocal
rem         :: "create_string" refers to create_string.bat
rem         :: under the same folder this file is under.
rem         call create_string var "Text in quotos"
rem         echo !var!
rem endlocal
rem goto :eof

rem E59//Continuing create_string function
rem setlocal EnableDelayedExpansion
rem goto :main
...
rem :main
rem setlocal
rem         call create_array var " " "This is some text"
rem         echo !var!
rem endlocal
rem goto :eof


rem E62//Delayed Variable Expansion without setlocal

:: see enabledelayedexpansion.bat

rem E63//Testing if DelayedVariableExpansion is on

:: single quotes in set (' ') means there's command in it.
rem for /f "tokens=*" %%a in ('reg query "HKLM\Software\Microsoft\Command Processor" /v "DelayedExpansion" ') do (
rem     set output=%%a
rem )
...
rem     call create_array list_of_numbers "," "3,-6,5,10,-5,3"
...
rem E65//Maximum Function
rem setlocal enabledelayedexpansion 
rem goto :main
rem :main
rem setlocal
rem     call create_array list_of_numbers "," "3,-6,5,10,-5,3"
rem     call get_maximum list_of_numbers max
rem     echo !list_of_numbers!
rem     echo !max!
...
rem E66//Power Function

rem setlocal enabledelayedexpansion 
rem goto :main
rem :main
rem setlocal
rem     call get_power 2 10 power
rem     echo !power!
...
rem E67//Absolute Value Function
rem setlocal enabledelayedexpansion 
rem goto :main
rem :main
rem setlocal
rem     call get_abs 2 abs 
rem     echo !abs!
        
rem endlocal
rem goto :eof

rem E68//Range Function
rem setlocal enabledelayedexpansion 
rem goto :main
rem :main
rem setlocal
rem     call range list "," 15 30 5
rem     echo !list!
rem     echo !list_length!
rem     echo !list[0]!
rem     echo !list[0]_length!
        
rem endlocal
rem goto :eof


rem E69//Sum Function
...
rem    call create_array list "," "1,2,3"  
rem    call sum list list_sum
rem    echo !list_sum!

rem endlocal
rem goto :eof

rem E70//Sort Function

:: This course stops here.
:: The rest is here: https://www.youtube.com/watch?v=JbAyxKjUPs0&list=PL69BE3BF7D0BB69C4
